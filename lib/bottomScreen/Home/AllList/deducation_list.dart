...
class DeducationScreen extends StatefulWidget {
  ...
  @override
  State<DeducationScreen> createState() => _ExpensesScreenState();
}
...
class _ExpensesScreenState extends State<DeducationScreen>
  ...
  Future<void> fetchExpenseData() async {
    ...
    final response = await http.get(
      Uri.parse(ApiRoutes.getDeductionList),
  ...
  // New: Delete deducation
  // Future<void> deleteAllowance(String allowanceId) async {
  //   final prefs = await SharedPreferences.getInstance();
  //   final token = prefs.getString('token');
  //
  //   final response = await http.post(
  //     Uri.parse('${ApiRoutes.deleteDeductionList}/$allowanceId'), // Adjust endpoint
  //     headers: {'Authorization': 'Bearer $token'},
  //   );
  //
  //   if (response.statusCode == 200) {
  //     // ScaffoldMessenger.of(context).showSnackBar(
  //     //   const SnackBar(content: Text('deducation deleted successfully')),
  //     // );
  //     fetchExpenseData(); // Refresh data
  //   } else {
  //     // ScaffoldMessenger.of(context).showSnackBar(
  //     //   const SnackBar(content: Text('Failed to delete deducation')),
  //     // );
  //   }
  // }
  ...
  Future<void> deleteAllowance(String allowanceId, String password) async {
    ...
    final response = await http.post(
      Uri.parse('${ApiRoutes.deleteDeductionList}/$allowanceId'),
      headers: {
  ...
  // New: Edit deducation
  Future<void> editAllowance(Map deducation) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('token');

    final response = await http.put(
      Uri.parse('${ApiRoutes.updateDeductionList}/${deducation['id']}'), // Adjust endpoint
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
      body: json.encode({
        'name': deducation['name'],
        'type': deducation['type'],
        'amount': deducation['amount'],
        'percentange': deducation['percentange'],
        'status': deducation['status'],
      }),
    );

    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('deducation updated successfully')),
      );
      fetchExpenseData(); // Refresh data
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to update deducation')),
      );
    }
  }
  ...
  void _filterExpenses() {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        filteredExpenses = deducation.where((deducation) {
          return deducation['amount'].toString().toLowerCase().contains(query) ||
              deducation['name'].toLowerCase().contains(query) ||
              deducation['percentange'].toString().toLowerCase().contains(query) ||
              deducation['expance_date'].toString().toLowerCase().contains(query);
        }).toList();
      }
      _animationController.forward(from: 0);
    });
              ...
              FadeTransition(
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        'Deduction List (${filteredExpenses.length})',
                        style: GoogleFonts.poppins(
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: List.generate(filteredExpenses.length, (index) {
                      final deducation = filteredExpenses[index];
                      return DataRow(
                              ...
                              child: Chip(
                                label: Text(
                                  ...
                                  style: GoogleFonts.poppins(
                                    fontSize: 14.sp,
                                    fontWeight: FontWeight.w600,
                                    color: deducation['status'] == 1
                                        ? HexColor('#069c06')
                                        : Colors.red.shade600,
                                  ),
                                ),
                                backgroundColor: deducation['status'] == 1
                                    ? HexColor('#E6F7E6')
                                    : Colors.red.shade50,
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                IconButton(
                                  ...
                                  onPressed: () => _showDeleteConfirmation(deducation['id'].toString()),
                                  // onPressed: () => _showPaymentCanceledDialog(context,deducation['id'].toString()),
                                ),
                              ],
                            ),
                          ),
